        ...
        <div className="flex items-start gap-4 mb-8">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              将 LogLens 接入 AI 工具（Cursor / Claude / Codex 等），让 AI 直接查询你的云日志。
              提供 <code className="px-1.5 py-0.5 rounded text-xs font-mono"
                style={{ background: 'var(--bg-overlay)', color: 'var(--text-primary)' }}>
                list_cloud_projects
              </code> 和 <code className="px-1.5 py-0.5 rounded text-xs font-mono"
                style={{ background: 'var(--bg-overlay)', color: 'var(--text-primary)' }}>
                search_cloud_logs
              </code> 两个工具。
            </p>
...
const MCP_TOOLS = [
  {
    ...
    label: '列出云项目',
    desc: '【第一步调用】返回 LogLens 中所有已配置的云日志项目及其日志流别名，用于发现可查询的对象。',
    params: [],
  ...
  },
  {
    ...
    label: '搜索云日志',
    desc: '按项目名 + 日志流别名直接查询云日志。支持华为云 LTS、阿里云 SLS、腾讯云 CLS，无需文件路径。',
    params: [
      { name: 'project_name', type: 'string', required: true,  desc: '云项目名称（来自 list_cloud_projects 的 name 字段）' },
      { name: 'alias_name',   type: 'string', required: true,  desc: '日志流别名（来自 list_cloud_projects 的 aliases[].alias 字段）' },
      { name: 'query',        type: 'string', required: false, desc: '查询关键词，如 "ERROR"、"payment timeout"、"userId=123 AND ERROR"。留空返回全部。' },
      { name: 'time_range',   type: 'string', required: false, desc: '"15m" | "1h" | "3h" | "24h" | "7d"，默认 1h' },
...
function McpToolsPanel() {
  const [expanded, setExpanded] = useState<string | null>('search_logs')

  return (
    <div className="flex flex-col gap-3">
      <div className="text-sm mb-2" style={{ color: 'var(--text-secondary)' }}>
        LogLens MCP 服务器对外暴露以下工具，AI 客户端可直接调用：
      </div>
      {MCP_TOOLS.map((tool) => (
        <div
          key={tool.name}
...
function McpDocsPanel() {
  ...
  const cursorJson = JSON.stringify({
    mcpServers: {
      loglens: {
        command: '/Applications/loglens.app/Contents/MacOS/loglens',
        args: ['--mcp-server'],
      },
    },
  }, null, 2)
...
让 AI 客户端（Cursor、Claude Desktop 等）能够直接调用 LogLens 的日志搜索能力。

可以把 MCP 理解为 Java 中的远程接口（RPC）——AI 作为"调用方"，LogLens 作为"服务提供方"。`,
    },
    {
      title: '快速接入步骤',
      steps: [
        ...
        '在 AI 对话中尝试："帮我搜索 /var/log/app.log 中的 ERROR 日志"',
      ],
    },
    {
      title: '手动配置（Cursor）',
      code: `// 文件: ~/.cursor/mcp.json\n${cursorJson}`,
    },
    {
      title: '手动配置（Claude Desktop）',
      code: `// 文件: ~/Library/Application Support/Claude/claude_desktop_config.json\n${cursorJson.replace('loglens', 'loglens')}`,
    },
    {
      title: '可用工具说明',
      table: [
        { name: 'search_logs', desc: '全文搜索日志，支持级别过滤、时间范围' },
        { name: 'get_log_stats', desc: '获取日志统计（各级别数量、服务分布）' },
        { name: 'get_log_file_info', desc: '获取文件元信息（行数、格式、时间跨度）' },
        { name: 'get_log_context', desc: '获取特定行号前后的上下文' },
        { name: 'analyze_anomalies', desc: 'AI 分析日志异常（需要配置 AI Provider）' },
      ],
    },
    {
      title: '示例对话',
      examples: [
        '帮我搜索 /var/log/app.log 中最近的 ERROR 日志',
        '分析 /home/app/logs/service.log 的日志统计',
        '找到 /logs/backend.log 第 1024 行前后的上下文',
        '这个日志文件 /var/log/nginx/access.log 里有什么异常？',
      ],
    },
    {
      title: '故障排查',
...
const TOOL_COLORS: Record<string, string> = {
  search_logs:       'bg-blue-500/15 text-blue-400 border border-blue-500/25',
  get_log_stats:     'bg-green-500/15 text-green-400 border border-green-500/25',
  get_log_file_info: 'bg-teal-500/15 text-teal-400 border border-teal-500/25',
  get_log_context:   'bg-violet-500/15 text-violet-400 border border-violet-500/25',
  analyze_anomalies: 'bg-orange-500/15 text-orange-400 border border-orange-500/25',
  list_log_files:    'bg-slate-500/15 text-slate-400 border border-slate-500/25',
}
...
function McpActivityPanel() {
  ...
  const tools = ['list_cloud_projects', 'search_cloud_logs']
