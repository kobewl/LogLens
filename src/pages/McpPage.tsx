          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              ...
              提供 <code className="px-1.5 py-0.5 rounded text-xs font-mono"
                style={{ background: 'var(--bg-overlay)', color: '#4ade80' }}>
                search_local_logs
              </code>、<code className="px-1.5 py-0.5 rounded text-xs font-mono"
                style={{ background: 'var(--bg-overlay)', color: 'var(--text-primary)' }}>
                get_log_context
              </code>、<code className="px-1.5 py-0.5 rounded text-xs font-mono"
                style={{ background: 'var(--bg-overlay)', color: '#60a5fa' }}>
                search_cloud_logs
              </code> 等 {MCP_TOOLS.length} 个工具。
            </p>
          </div>
        </div>
...
const MCP_TOOLS = [
  {
    ...
    label: '📂 列出日志文件',
    category: 'local',
    desc: '【推荐第一步调用】列出 LogLens 中最近打开过的本地日志文件。AI Agent 应先调用此工具发现可用的日志文件，然后再用 search_local_logs 具体搜索。',
    params: [],
    workflow: '列出可用日志文件 → 获取 file_path → search_local_logs / get_log_stats',
  },
  {
    name: 'search_local_logs',
    label: '🔍 搜索本地日志',
    category: 'local',
    desc: '【核心工具】搜索本地日志文件，支持全文检索和字段过滤（level=ERROR, service=api等）。这是 AI Agent 调试时最常用的工具。',
    params: [
      { name: 'file_path', type: 'string', required: true,  desc: '日志文件的绝对路径，如 /var/log/app.log。可从 list_log_sessions 获取。' },
      { name: 'query',     type: 'string', required: false, desc: '搜索查询。示例："ERROR"、"level=ERROR"、"level=ERROR AND service=payment"、"timeout"。留空返回所有行。' },
  ...
  },
  {
    ...
    label: '📋 上下文查看',
    category: 'local',
    desc: '获取指定日志行前后的上下文行。当 search_local_logs 发现一个错误行时，用此工具查看它前后发生了什么。',
    params: [
  ...
  },
  {
    ...
    label: '📊 日志统计',
    category: 'local',
    desc: '获取日志文件的高级统计概览：总行数、时间范围、各级别分布、各服务分布、时间线摘要。AI Agent 用此工具快速了解日志文件的全貌。',
    params: [
      { name: 'file_path', type: 'string', required: true,  desc: '日志文件的绝对路径' },
    ],
  ...
  },
  {
    ...
    label: '☁️ 列出云项目',
    category: 'cloud',
    desc: '列出 LogLens 中所有已配置的云日志项目（华为云 LTS / 阿里云 SLS / 腾讯云 CLS）及其日志流别名。',
    params: [],
  ...
  },
  {
    ...
    label: '☁️ 搜索云日志',
    category: 'cloud',
    desc: '按项目名 + 日志流别名直接查询云端日志。支持华为云 LTS、阿里云 SLS、腾讯云 CLS，无需文件路径。',
    params: [
...
function McpActivityPanel() {
  ...
  const tools = ['search_local_logs', 'get_log_context', 'list_log_sessions', 'get_log_stats', 'list_cloud_projects', 'search_cloud_logs']
