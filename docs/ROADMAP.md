...
| **v0.3** | Month 4 | + tantivy 全文索引 + 搜索 + 统计 | 超越 glogg，达到 lnav 搜索能力 |
| **v0.5** | Month 6 | + AI 异常检测 + 自然语言查询 | **独有**：AI 驱动的日志分析 |
| **v0.7** | Month 8 | + 多文件 + 实时追踪 + 告警 | 达到 Dozzle 实时能力 + 持久化 |
...
1. **MCP Server**：让 AI Agent 能直接搜你的日志
2. **插件系统**：社区贡献新的日志格式解析器
3. **日志对比**：两个时间段的日志 Diff
4. **异常检测模型训练**：基于用户标记的异常，微调检测模型
5. **报告生成**：一键生成 PDF 日志分析报告
6. **Web 自托管**：团队共享日志分析结果
